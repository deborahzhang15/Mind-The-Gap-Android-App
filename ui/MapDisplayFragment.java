...
//    private GestureDetector.OnDoubleTapListener odtl;
    ...
    private void plotLines(List<Line> tubeLines) {
        for(Line tl : tubeLines) {
            tl.getColour();
            Set<Branch> brnchs = tl.getBranches();
            for(Branch b : brnchs){
                Polyline polyline = new Polyline(mapView.getContext());
                List<LatLon> pts = b.getPoints();
                List<GeoPoint> gpts = new ArrayList<GeoPoint>();
                for(LatLon pt:  pts){
                    GeoPoint gp = new GeoPoint(pt.getLatitude(),pt.getLongitude());
                    gpts.add(gp);
                }
                polyline.setColor(tl.getColour());
                polyline.setWidth(getLineWidth(mapView.getZoomLevel()));
                polyline.setPoints(gpts);
    ...
    private void handleLocationChange(Location location) {
        LatLon pt = new LatLon(location.getLatitude(),location.getLongitude());
        Station closest = StationManager.getInstance().findNearestTo(pt);
